...
///
/// https://contest.yandex.ru/contest/24810/run-report/88555351/
///=======================================================================
/// -- ПРИНЦИП РАБОТЫ --
/// Для решения данной задачи массив объектов перераспределяется в бинарную кучу.
...
///
/// Индекс родителя узла i равен [i/2] ([] - означает взятие целой части числа);
/// Индекс левого потомка узла i равен 2i;
/// Индекс правого потомка узла i равен 2i + 1;
///
/// Т.о. преобразуя массив к приоритетной бинарной куче мы может отсортировать массив по необходимым параметрам
/// (в данной случае сортировка производится по комплексному параметру, описанному в тексе задачи).
///
/// -- ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ --
/// Корректность работы алгоритма достигается за счет перераспределения (просеивания) массива.
/// В результате просеивания достигается порядок элементов, при котором в вершине дерева находится 
/// элемент с наибольшим приоритетом, чем у потомков.
///
/// -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
/// Временная сложность пирамидальной сортировки - O(nlogn).
///
